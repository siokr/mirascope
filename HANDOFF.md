...
`M03-001`、`M03-002`、`M03-004` 至 `M03-009` 已完成；`M03-003` 的 ZIP/CBZ 与 Windows 目录身份及重定位已实现，Android 目录因现有插件不保留 SAF URI 权限而保持进行中。漫画阅读器现可恢复章节、章内页、页内比例、阅读模式和翻页方向，并用 revision/时间戳防止旧写覆盖，368 项全量测试通过。下一项推进 `M03-010` 预加载与分层缓存。在线漫画源、站点规则、下载、番剧、账号和同步不属于 0.3。
...
`v0.2.0` 已正式发布；Version 0.3 的本地漫画阅读闭环已包含页级进度和独立阅读偏好；Android 目录持久权限保持显式阻塞，下一步执行 `M03-010` 预加载与分层缓存。
