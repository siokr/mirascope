...
`M03-001`、`M03-002`、`M03-004` 至 `M03-010` 已完成；`M03-003` 的 ZIP/CBZ 与 Windows 目录身份及重定位已实现，Android 目录因现有插件不保留 SAF URI 权限而保持进行中。漫画阅读器已具备页级状态、相邻页预加载、快速翻页取消，以及 64 MiB 内存/512 MiB 磁盘的内容身份缓存，374 项全量测试通过。下一项推进 `M03-011` 缓存清理、来源恢复与异常闭环。在线漫画源、站点规则、下载、番剧、账号和同步不属于 0.3。
...
`v0.2.0` 已正式发布；Version 0.3 的本地漫画阅读闭环已包含页级状态、预加载和有界分层缓存；Android 目录持久权限保持显式阻塞，下一步执行 `M03-011` 缓存清理、来源恢复与异常闭环。
