...
`M03-001`、`M03-002`、`M03-004` 至 `M03-007` 已完成；`M03-003` 的 ZIP/CBZ 与 Windows 目录身份及重定位已实现，Android 目录因现有插件不保留 SAF URI 权限而保持进行中。漫画已接入媒体库、派生封面、详情、章节页数、来源缺失、同指纹重定位、归档和删除闭环，359 项全量测试通过。下一项推进 `M03-008` 漫画阅读器。在线漫画源、站点规则、下载、番剧、账号和同步不属于 0.3。
...
`v0.2.0` 已正式发布；Version 0.3 的本地漫画导入、媒体库、详情、目录及来源生命周期已形成闭环；Android 目录持久权限保持显式阻塞，下一步执行 `M03-008` 漫画阅读器。
