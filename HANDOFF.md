...
`M03-001`、`M03-002`、`M03-004`、`M03-005` 已完成；`M03-003` 的 ZIP/CBZ 与 Windows 目录身份已实现，Android 目录因现有插件不保留 SAF URI 权限而保持进行中。目录与 ZIP/CBZ 已统一完成真实图片解码、自然排序和稳定章节/页清单，343 项全量测试通过。下一项推进 `M03-006` 派生存储、缩略图与原子导入。在线漫画源、站点规则、下载、番剧、账号和同步不属于 0.3。
...
`v0.2.0` 已正式发布；Version 0.3 已完成依赖审计、schema v4、安全 ZIP/CBZ 容器、图片真实验证和稳定清单；Android 目录持久权限保持显式阻塞，下一步执行 `M03-006` 派生存储、缩略图与原子导入。
