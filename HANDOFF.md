...
`M03-001`、`M03-002`、`M03-004`、`M03-005`、`M03-006` 已完成；`M03-003` 的 ZIP/CBZ 与 Windows 目录身份已实现，Android 目录因现有插件不保留 SAF URI 权限而保持进行中。漫画缩略图、固化清单、媒体/书架/章节/页/导入记录已形成带补偿清理的原子导入，352 项全量测试通过。下一项推进 `M03-007` 媒体库、漫画详情与目录。在线漫画源、站点规则、下载、番剧、账号和同步不属于 0.3。
...
`v0.2.0` 已正式发布；Version 0.3 已完成依赖审计、schema v4、安全容器、图片验证、稳定清单及漫画原子导入；Android 目录持久权限保持显式阻塞，下一步执行 `M03-007` 媒体库、漫画详情与目录。
