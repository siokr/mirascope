...
`M03-001`、`M03-002`、`M03-004` 至 `M03-008` 已完成；`M03-003` 的 ZIP/CBZ 与 Windows 目录身份及重定位已实现，Android 目录因现有插件不保留 SAF URI 权限而保持进行中。漫画已形成导入、媒体库、详情和真实阅读路由，支持纵向连续、横向单页、左右翻页方向、目录、键盘、缩放及坏页占位，363 项全量测试通过。下一项推进 `M03-009` 页级进度与阅读偏好。在线漫画源、站点规则、下载、番剧、账号和同步不属于 0.3。
...
`v0.2.0` 已正式发布；Version 0.3 的本地漫画已能从导入进入两种模式的真实阅读器；Android 目录持久权限保持显式阻塞，下一步执行 `M03-009` 页级进度与阅读偏好。
