...
`M03-001`、`M03-002` 已完成；`M03-003` 的 ZIP/CBZ 与 Windows 目录身份已实现，Android 目录因现有插件不保留 SAF URI 权限而保持进行中。下一项可独立推进 `M03-004` 安全 ZIP/CBZ 容器。在线漫画源、站点规则、下载、番剧、账号和同步不属于 0.3。
...
`v0.2.0` 已正式发布；Version 0.3 的依赖审计和 schema v4 已完成，漫画 ZIP/CBZ 与普通目录身份已实现；Android 目录持久权限保持显式阻塞，下一步执行可独立验证的 `M03-004` 安全 ZIP/CBZ 容器。
