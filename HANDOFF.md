...
`M03-001`、`M03-002`、`M03-004` 已完成；`M03-003` 的 ZIP/CBZ 与 Windows 目录身份已实现，Android 目录因现有插件不保留 SAF URI 权限而保持进行中。安全 ZIP/CBZ 容器已具备按需读取、路径防护、CRC 校验及资源预算，20 项专项和 332 项全量测试通过。下一项推进 `M03-005` 图片验证、自然排序与章节清单。在线漫画源、站点规则、下载、番剧、账号和同步不属于 0.3。
...
`v0.2.0` 已正式发布；Version 0.3 的依赖审计、schema v4 和安全 ZIP/CBZ 容器已完成，漫画 ZIP/CBZ 与普通目录身份已实现；Android 目录持久权限保持显式阻塞，下一步执行 `M03-005` 图片验证、自然排序与章节清单。
