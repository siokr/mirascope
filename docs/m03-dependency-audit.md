...
`file_selector_android 0.5.2+9` 的 `getDirectoryPath` 使用 `ACTION_OPEN_DOCUMENT_TREE`，随后将系统返回的文档树 URI 转换为普通主存储路径。当前实现没有取得持久化 URI 权限，并明确不支持非主存储卷和其他文档提供者。因此“API 表列出 Android 目录选择”只证明能打开选择界面，不能证明应用重启后仍能可靠读取所选目录。

当前决定：ZIP/CBZ 选择与指纹、Windows 普通目录选择与指纹继续使用现有依赖；Android 目录支持保持未完成，后续需评估最小的 SAF URI 持久化适配及其许可证、迁移和来源生命周期，不通过复制整个用户目录绕过权限语义。
