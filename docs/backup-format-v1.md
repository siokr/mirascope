...
恢复数据库主文件时必须在同一事务内移走旧的 `-wal`、`-shm` 和 `-journal` 旁路文件；成功后不保留这些旧文件，失败回滚时与数据库主文件一并恢复。

## 导出一致性

运行中的数据库使用 SQLite `VACUUM INTO` 生成独立一致性快照，不直接复制可能仍依赖 WAL 的数据库主文件。ZIP 先写入目标目录下的独立临时目录，完成清单校验后才重命名为最终目标；目标文件已经存在时拒绝覆盖。
